...
/** Mac App Store button. Links to the storefront matching the current language. */
export default function AppStoreButton() {
  const { appStore } = useCopy();
  const { lang } = useLang();

  return (
    <a className="appstore-btn" href={appStoreUrl(lang)} target="_blank" rel="noreferrer">
      <svg viewBox="0 0 384 512" aria-hidden="true">
        <path d={APPLE_LOGO} />
      </svg>
      <span className="lbl">
        <small>{appStore.small}</small>
        <strong>{appStore.download}</strong>
      </span>
    </a>
  );
}
